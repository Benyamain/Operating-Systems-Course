...
setptrs(int bufsize, char buf[], char *ptk[])
{
    ...
    while (buf_idx < bufsize) {
        if (!input_tkn && buf[buf_idx] != ' ' && buf[buf_idx] != '|' && buf[buf_idx] != '\n') {
            input_tkn = 1;
            ptk[++tok_idx] = &buf[buf_idx];
        } else if (input_tkn && (buf[buf_idx] == ' ' || buf[buf_idx] == '|' || buf[buf_idx] == '\n')) {
            input_tkn = 0;
            buf[buf_idx] = '\0';
        ...
        } else if (!input_tkn && (buf[buf_idx] == '|' || buf[buf_idx] == '\n')) {
		buf[buf_idx] = '\0';
		ptk[++tok_idx] = NULL;
	}
        buf_idx++;
