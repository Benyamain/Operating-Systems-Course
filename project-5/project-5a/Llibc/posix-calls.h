...
int Lcreat(const char *pathname, mode_t mode);
...
int Lgetcwd(char *buf, unsigned int len);
int Ldup(int oldfd);
int Ldup2(int oldfd, int newfd);
int Lgetpid(void);
int Lpause(void);
unsigned int Lsleep(unsigned int decisecs); /* Non-POSIX, as in xv6 */
int Luptime(void);              /* Not implemented, non-POSIX */
