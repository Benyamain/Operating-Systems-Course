...
/* Allocate storage for __Largc, __Largv, __Lenvp */
int __Largc;
char ** __Largv;
char ** __Lenvp;
    ...
    while ((c = *format++) != 0) {
        if (c != '%')
            ...
            switch (c) {
                ...
                case 'u':
                case 'x':
                    if (flushleft == 1 && pad0 == 1)
                        /* Can't pad a number on the right with 0s, so we must
                        turn off either the '-' flag or the '0' flag.  GNU and
                        man printf:  "A - overrides a 0 if both are given." */
                        pad0 = 0;   // flushleft = 0; /* would be better */
                    if (longflag == 1) {
...
/* Allow any string not containing '=' as env var name (even empty string!) */
const char *
Lgetenv(const char *s)
{
    char *p;
    char **ep = __Lenvp;
    int slen = Lstrlen((char *) s);
    for (int k = 0; ep[k] != 0; k++) {
        p = Lstrchr(ep[k], '=');
        if (p == 0 /* no '=' */ || ((int) (p - ep[k])) != slen /* wrong len */)
            continue;
        /* Now manually do strncmp(s, ep[k], slen) */
        int j;
        for (j = 0; j < slen && *(ep[k] + j) == s[j]; j++)
            ;
        if (j == slen) /* match! */
            return ep[k] + j + 1;
    }
...
#define MAXPATHLEN  4096        /* POSIX suggested */
int
Lexecvp(char *prog, char **args)
{
    char path[MAXPATHLEN + 1];
    const char *p = Lgetenv("PATH");

    /* If the string prog contains '/', just pass to kernel execve syscall */
    if (Lstrchr(prog, '/') != 0 || p == 0 /* PATH env var not found */)
        return Lexecve(prog, args, __Lenvp);

    /* Env var PATH found, no '/' in prog - search and form parh for kernel */
    int plen, fd;
    while ((plen = Lstrlen((char *) p)) > 0) {
        char *q = Lstrchr(p, ':');
        if (q == 0)
            q = (char *) p + plen;
        int k;
        for (k = 0; k < MAXPATHLEN && p < q; k++)
            path[k] = *p++;
        /* Now p = q */
        p++;
        path[k++] = '/';
        q = prog;
        for (; k < MAXPATHLEN && *q != 0; k++)
            path[k] = *q++;
        path[k] = 0;
        /* Lfprintf(2, "DEBUG: %s\n", path); */
        if ((fd = Lopen(path, O_RDONLY)) >= 0) {
            Lclose(fd);
            return Lexecve(path, args, __Lenvp);
        }
    }

    /* If here, there was no match after prefixing with paths in PATH */
    return Lexecve(prog, args, __Lenvp);
