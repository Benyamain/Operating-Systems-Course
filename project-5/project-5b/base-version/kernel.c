...
/* Assembly routines from cs-riscv64.S needed here */
...
Lmain(int argc, char *bootparms[])
{
  ...
  /* Call queinit(), which initializes the two queues (freeList, readyQueue)
     and creates the dummy initial process P0.  P0 will never run any code,
     but queinit() will set it as "running" so that we can use P0 as the
     first outgoing process to switch out --- P0 will never "run" again!
  */
  ...
  /* Standardized kfork() doesn't initialize the child (just clones
     from the parent), so if we want a child forked from P0 to be
  ...
  */
  ...
  /* Now a child of P0 made via standardized kfork() will have code to run! */
...
{
    /* Add YOUR CODE here!  Match with output of kernel.prebuilt (5A) */
